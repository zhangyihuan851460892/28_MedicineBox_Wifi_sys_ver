...
void RTC_CheckAndConfig(rtc_time *tm)
{
	...
	else
	{
	  /*启动无需设置新时钟*/
		...
		//如果需要后续设置时钟，则需要开放BKP和PWR寄存器，否则RTC_WaitForLastTask出不来
...
/*
 * 函数名：Time_RegulateByInternet
 * 描述  ：通过internet设置时钟，返回用户在超级终端中输入的时间值，并将值储存在
 *         RTC 计数寄存器中。
 * 输入  ：timeVar 通过internet获取的时间戳
 * 输出  ：用户在超级终端中输入的时间值，单位为 s
 * 调用  ：内部调用
 */
...
//	   uint8_t str[15]; // 字符串暂存  	
	  ...
	  printf(" UNIX时间戳 = %d 当前时间为: %d年(%s年) %d月 %d日 (星期%s)  %0.2d:%0.2d:%0.2d\r\n",TimeVar,
		...
		LCD_DisplayHours(tm->tm_hour,tm->tm_sec%2);
		LCD_DisplayMinutes(tm->tm_min);
}
