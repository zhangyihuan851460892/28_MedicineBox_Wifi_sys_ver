...
//		if(!(STM_EVAL_PBGetState(BUTTON_RIGHT)) || !(STM_EVAL_PBGetState(BUTTON_LEFT)) || !(STM_EVAL_PBGetState(BUTTON_KEY))) break;
  }
  return key;
...
void Main_Menu(void)
{
  ...
  {
    SerialPutString("\r\n====================== Main Menu ============================\r\n\n");
    SerialPutString("  下载用户程序                                       ------- 1\r\n\n");
    SerialPutString("  清空E2PROM                                         ------- 2\r\n\n");
    ...
    SerialPutString("  测试LED和霍尔开关            ----------------------------- 5\r\n\n");
		SerialPutString("  测试录音和删除录音           ----------------------------- 6\r\n\n");
    SerialPutString("==============================================================\r\n\n");
    ...
    else if (key == 0x32)  //清空eeprom
    {
 //     /* Upload user application from the Flash */
 //     SerialUpload();
			Delay_ms(10);
//			I2C_EE_BufferWrite(I2c_Buf_Write, 0, 192);
			AT24CXX_Write(0,I2c_Buf_Write,192);
			Delay_ms(400);
			SerialPutString("Clear Device Authorize Flag Success!\r\n\n");
    }
    ...
    else if ((key == 0x34) && (FlashProtection == 1))
    {
      ...
      FLASH_DisableWriteProtectionPages();
		...
		else if (key == 0x35)  //测试霍尔开关
		{
			BEEP();
			TestLEDandSWITCH();
		}
		else if (key == 0x36)  //测试录音
		{
			BEEP();
			TestRECandERASE();
		}
